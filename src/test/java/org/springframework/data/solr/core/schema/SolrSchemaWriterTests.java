...
public class SolrSchemaWriterTests {
	...
	public void schemaPresentCheckSouldIndicateFalseWhenServerReturnsNotFound() throws SolrServerException, IOException {
		...
		Mockito.when(solrClientMock.request(Mockito.any(SolrRequest.class), Matchers.anyString())).thenThrow(
	...
	private void setUpJsonResponse(String json) {
		...
		try {
			Mockito.when(solrClientMock.request(Mockito.any(SolrRequest.class), Matchers.anyString())).thenReturn(namedList);
		} catch (Exception e) {}
	}
}
